...
%% Articulation angle       
            ...
            %probando=sum(isnan(Angle)
            elseif (sum(isnan(Angle))>0) || (max(Angle) > Angle_joint ) %Maximum Angle 60° 
                                %V1= H*pi*R^2;
                                %V2= L4*pi*L1^2;
                                %Hc=sqrt(L2^2-(L1-L3)^2);
                                %V3= (1/3)*pi*Hc*(L1^2+L3^2+L1*L3); % 
                                %Penalty=(Penal*((round(V1)+ round(V2)+ round(V3))))/100;
                                %F= (round(V2)+ round(V3)+ round(V1))* Penalty;                                     
                                %Save_result(F,L1,L2,L3,L4);                                                   
                                F=1984
                                Angle
            ...
            %end
                ...
                if (R_Draw == 1)
                    ...
                    hold on
                    plot3(a,b,c,'LineWidth',1,'MarkerEdgeColor',[0.6,0.85,0.91])%% dibujar trayectoria
                    %plot3(a,b,c,'.',1,'Color','magenta')%% dibujar trayectoria
                   
%                     plot3(a,b,c,'*','LineWidth',1,'MarkerEdgeColor',[0.2,0.2,0.4],...
%                      'MarkerFaceColor','y','MarkerSize',4)%% Draw path *
                    
                    plot3(a,b,c,'.','LineWidth',1,'MarkerEdgeColor',[0.23,0.42,0.76],...
                                'MarkerFaceColor','y','MarkerSize',8)%% Draw path
...
% V3= (1/3)*pi*Hc*(L1^2+L3^2+L1*L3); % 
% Penalty=1;
%F= (round(V2)+ round(V3)+(L1*L2*L3*L4))* Penalty; 
F= 2009
% %F= (round(2*V2)+ round(V3))+ Penalty 
% Save_result(F,L1,L2,L3,L4);
